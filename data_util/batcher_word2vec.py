...
from . import config_word2vec
from . import data_word2vec
...
class Example(object):
    def __init__(self, article, abstract_sentences, vocab):
        ...
        start_decoding = vocab.word2id(data_word2vec.START_DECODING)
        stop_decoding = vocab.word2id(data_word2vec.STOP_DECODING)

        # Process the article
        article_words = article.split()
        if len(article_words) > config_word2vec.max_enc_steps:
            article_words = article_words[:config_word2vec.max_enc_steps]
        self.enc_len = len(
            article_words
        ...
        # article_words = 'scu 寿命 头孢'
        # print('**************article words******************')
        # print(article_words)
        # 每个article_words输出一则新闻
        self.enc_input_extend_vocab, self.article_oovs = data_word2vec.article2ids( article_words, vocab )
        # print('**************************')
        # print(self.article_oovs)
        # print(data_word2vec.show_art_oovs( article_words, vocab ))    #查看文章的oov
        # Get a verison of the reference summary where in-article OOVs are represented by their temporary article OOV id
        abs_ids_extend_vocab = data_word2vec.abstract2ids(abstract_words, vocab, self.article_oovs)
...
class Batch(object):
    def __init__(self, example_list, vocab, batch_size):
        # example_List: 来自于训练集 config_word2vec.train_data_path
        self.batch_size = batch_size
        self.pad_id = vocab.word2id(data_word2vec.PAD_TOKEN)  # id of the PAD token used to pad sequences
    ...
    def init_decoder_seq(self, example_list):
        ...
        for ex in example_list:
            ex.pad_decoder_inp_targ(config_word2vec.max_dec_steps, self.pad_id)

        # Initialize the numpy arrays.
        self.dec_batch = np.zeros((self.batch_size, config_word2vec.max_dec_steps),
                                  dtype=np.int32)
        self.target_batch = np.zeros((self.batch_size, config_word2vec.max_dec_steps),
                                     dtype=np.int32)
        # self.dec_padding_mask = np.zeros((self.batch_size, config_word2vec.max_dec_steps), dtype=np.float32)
    ...
    def next_batch(self):
        ...
        if self._batch_queue.qsize() == 0:
            ...
            if self._single_pass and self._finished_reading:
                ...
                return None
        # print('natcher.next_batch')
        batch = self._batch_queue.get()  # get the next Batch
        # print('next batch get')
        return batch

    def fill_example_queue(self):
        input_gen = self.text_generator(data_word2vec.example_generator(self._data_path, self._single_pass))    # example_generator读取chunk后的二进制转化为acsii码，text_generator将其转化为text输入 每次读的是一则新闻还是一个bin文件？

        while True:
            try:
                (article, abstract) = next(input_gen)  # read the next example from file. article and abstract are both strings.
            except StopIteration:  # if there are no more examples:
                #logger.info("The example generator for this example queue filling thread has exhausted data_word2vec.")
                if self._single_pass:
                    #logger.info("single_pass mode is on, so we've finished reading dataset. This thread is stopping.")
                    self._finished_reading = True
                    break
                else:
                    raise Exception("single_pass mode is off but the example generator is out of data_word2vec; error.")

            # abstract_sentences = [sent.strip() for sent in data_word2vec.abstract2sents(abstract)] # Use the <s> and </s> tags in abstract to get a list of sentences.
            abstract_sentences = [abstract.strip()]
            example = Example(article, abstract_sentences, self._vocab)  # Process into an Example. 将新闻和摘要转化为Example形式，填入_example_queue中
            ...
            while True:
                ...
                try:
                    article_text = e.features.feature['article'].bytes_list.value[
                        0]  # the article text was saved under the key 'article' in the data_word2vec files
                    abstract_text = e.features.feature['abstract'].bytes_list.value[
                        0]  # the abstract text was saved under the key 'abstract' in the data_word2vec files
                    article_text = article_text.decode()
                    abstract_text = abstract_text.decode()    #转化为utf-8编码
                except ValueError:
