...
finished_path = "CLTS/finished"
unfinished_path = "CLTS/unfinished"
chunk_path = "CLTS/chunked"

vocab_path = "CLTS/vocab"
VOCAB_SIZE = 50000

CHUNK_SIZE = 100  # num examples per chunk, for the chunked data   CHUNK_SIZE = 100
...
if __name__ == "__main__":
    train_article = open('./CLTS/unfinished/train.src', 'r', encoding="utf-8")
    with open('./CLTS/unfinished/train.article.txt', 'w') as f:
        for article in train_article.readlines():
            f.write(preprocess(article))
            f.write('\n')
    train_title = open('./CLTS/unfinished/train.tgt', 'r', encoding="utf-8")
    with open('./CLTS/unfinished/train.title.txt', 'w') as f:
        for title in train_title.readlines():
            f.write(preprocess(title))
            f.write('\n')
    valid_article = open('./CLTS/unfinished/valid.src', 'r', encoding="utf-8")
    with open('./CLTS/unfinished/valid.article.txt', 'w') as f:
        for article in valid_article.readlines():
            f.write(preprocess(article))
            f.write('\n')
    valid_title = open('./CLTS/unfinished/valid.tgt', 'r', encoding="utf-8")
    with open('./CLTS/unfinished/valid.title.txt', 'w') as f:
        for title in valid_title.readlines():
            f.write(preprocess(title))
            f.write('\n')
    shuffle_text_data("train.article.txt", "train.title.txt", "train.art.shuf.txt", "train.abs.shuf.txt")
    shuffle_text_data("valid.article.txt", "valid.title.txt", "valid.art.shuf.txt", "valid.abs.shuf.txt")
    ...
    bin_chunks.sort()
    samples = random.sample(set(bin_chunks[:-1]), 2)     #samples = random.sample(set(bin_chunks[:-1]), 2)
