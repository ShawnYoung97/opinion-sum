...
# from torchscan  import summary #使用开源工具算复杂度
...
class Train(object):
    ...
    # ------------------------------------------------------------------------------------
        ...
        # 学习率衰减
        # self.scheduler.step()
        return mle_loss.item(), rl_loss.item(), batch_reward
    ...
    def trainIters(self):
        ...
        while iter <= config_word2vec.max_iterations:
            ...
            try:
                mle_loss, rl_loss, r = self.train_one_batch(batch, iter)
            ...
            if iter % 100== 0:    #if iter % 5000 == 0
                ...
                #stat(self.save_model(iter), 1500)   # Complexity
