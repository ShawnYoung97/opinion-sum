...
class Beam(object):
    def __init__(self, start_id, end_id, unk_id, hidden_state, context):
        ...
        self.tokens = T.LongTensor(config_word2vec.beam_size, 1).fill_(
            start_id)  #(beam, t) after t time steps
        self.scores = T.FloatTensor(config_word2vec.beam_size, 1).fill_(
        ...
        self.hid_h = h.unsqueeze(0).repeat(config_word2vec.beam_size, 1)  #beam, n_hid ，unsqueeze（1）在第一维上增加一个维度，repeat()对原来的第一个维度复制beam_size,第二个维度保持不变
        self.hid_c = c.unsqueeze(0).repeat(config_word2vec.beam_size, 1)  #beam, n_hid
        self.context = context.unsqueeze(0).repeat(config_word2vec.beam_size, 1)  #beam, 2*n_hid
    ...
    def get_current_state(self):
        ...
        for i in range(len(tokens)):
            if tokens[i].item() >= config_word2vec.vocab_size:
    ...
    def advance(self, prob_dist, hidden_state, context, sum_temporal_srcs,
        ...
        h, c = hidden_state
        log_probs = T.log(prob_dist + config_word2vec.eps)  #beam, n_extended_vocab
        ...
        best_scores, best_scores_id = T.topk(
            input=scores, k=config_word2vec.beam_size,
...
def beam_search(s_tensor, enc_hid, enc_out, enc_padding_mask, ct_e, extra_zeros,
    ...
    for t in range(config_word2vec.max_dec_steps):
        #(rem*beam,)#remaining(rem),beam，得到当前的id，view(-1)表示让计算机自己定义tensor的形状
        x_t = T.stack([beam.get_current_state() for beam in beams if (not beam.done)]).contiguous().view(-1)
        x_t = model.embeds(x_t)  #rem*beam, n_emb，得到当前输入的向量表示
        dec_h = T.stack([beam.hid_h for beam in beams
                         if (not beam.done)]  #rem*beam,n_hid
                        ).contiguous().view(-1, config_word2vec.hidden_dim)
        dec_c = T.stack([beam.hid_c for beam in beams
                         if (not beam.done)]  #rem,beam,n_hid
                        ).contiguous().view(-1, config_word2vec.hidden_dim)  #rem*beam,n_hid

        ct_e = T.stack([beam.context for beam in beams
                        if (not beam.done)]  #rem,beam,n_hid
                       ).contiguous().view(-1, 2 * config_word2vec.hidden_dim)  #rem,beam,n_hid
        ...
        if prev_s is not None:
            prev_s = T.stack([
                beam.prev_s for beam in beams if (not beam.done)
                                 ...
                                 config_word2vec.hidden_dim)  #rem*beam, t-1, n_hid

        s_t = (dec_h, dec_c)
        enc_out_beam = enc_out[beam_idx].view(n_rem, -1).repeat(1, config_word2vec.beam_size).view(-1, enc_out.size(1), enc_out.size(2))
        enc_pad_mask_beam = enc_padding_mask[beam_idx].repeat(1, config_word2vec.beam_size).view(-1, enc_padding_mask.size(1))

        extra_zeros_beam = None    # 这里是我自己注释掉的
        if extra_zeros is not None:
            extra_zeros_beam = extra_zeros[beam_idx].repeat(1, config_word2vec.beam_size).view(-1, extra_zeros.size(1))
        enc_extend_vocab_beam = enc_batch_extend_vocab[beam_idx].repeat(1, config_word2vec.beam_size).view(-1, enc_batch_extend_vocab.size(1))

        final_dist, (dec_h,dec_c), ct_e, au, et2, sum_temporal_srcs, prev_s = model.decoder(
        ...
        final_dist = final_dist.view(n_rem, config_word2vec.beam_size, -1)  #final_dist: rem, beam, n_extended_vocab
        dec_h = dec_h.view(n_rem, config_word2vec.beam_size, -1)  #rem, beam, n_hid
        dec_c = dec_c.view(n_rem, config_word2vec.beam_size, -1)  #rem, beam, n_hid
        ct_e = ct_e.view(n_rem, config_word2vec.beam_size, -1)  #rem, beam, 2*n_hid

        if sum_temporal_srcs is not None:
            sum_temporal_srcs = sum_temporal_srcs.view(n_rem, config_word2vec.beam_size,
                                                       -1)  #rem, beam, n_seq

        if prev_s is not None:
            prev_s = prev_s.view(n_rem, config_word2vec.beam_size, -1,
                                 config_word2vec.hidden_dim)  #rem, beam, t
